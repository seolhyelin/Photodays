...
const Dates = (props) => {
  const { lastDate, firstDate, elm, findToday, month, year, idx, holiday } = props;
          ...
          <Holidays>
            {holiday !== undefined &&
              holiday.map((evt, index) => {
                let key = elm.length < 2 ? `${year}` + `${month}` + `${elm}` : `${year}` + `${month}` + '0' + `${elm}`;
                return Number(key) === evt.locdate && <Holiday key={index}>{evt.dateName}</Holiday>;
              })}
          </Holidays>
        )}
        {Boolean(evtList[0]) && (
